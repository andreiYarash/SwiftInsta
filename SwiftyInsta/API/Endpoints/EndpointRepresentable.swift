...
public extension EndpointRepresentable {
    ...
    /// Query `minId`.
    func next(minId: String?) -> EndpointRepresentable! { return representation.next(minId: minId) }
    /// Query `rankToken`.
...
extension LosselessEndpointRepresentable {
    ...
    /// Query `minId`.
    func next(minId: String?) -> EndpointRepresentable! { return minId.flatMap { query(["min_id": $0]) } ?? self }
    /// Query `rankToken`.
