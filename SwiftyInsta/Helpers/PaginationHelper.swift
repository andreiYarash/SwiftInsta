...
class PaginationHelper: Handler {
    ...
    func request<Response, Page>(_ response: Response.Type,
                                 cursorType: PaginationHelper.CursorType = .max,
                                 page: Page.Type,
                                 ...
                                 next: @escaping (DynamicResponse) -> String? = {
                                        if let nextMaxID = $0.nextMaxId.string {
                                            return nextMaxID
                                        } else {
                                            return $0.nextMinId.string
                                        }
                                 },
        ...
        func requestNextPage() {
            ...
            handler.requests.request(page,
                                     ...
                                     process: process) { [weak self] in
                                        ...
                                        nextPaginationParameters.currentMaxId = cursorType == .max ? nextPaginationParameters.nextMaxId : nextPaginationParameters.nextMinId
                
                                        // update the pagination parameters.
                                        nextPaginationParameters.nextMaxId = nil
                                        nextPaginationParameters.nextMinId = nil
                                        // check for handler.
                                        ...
                                        case .success(let page):
                                            ...
                                            if cursorType == .max {
                                                ...
                                                nextPaginationParameters.nextMinId = nextPage(page)
                                            }
                                            ...
                                            guard !(nextPaginationParameters.nextMaxId ?? "").isEmpty || !(nextPaginationParameters.nextMinId ?? "").isEmpty else {
                                                return handler.settings.queues.response.async {
                                                    completion(.success(responses), nextPaginationParameters)
                                                }
                                            }
